...
fn main() {
    let fname = "inputs/day1/my.txt";
    let contents = fs::read_to_string(fname).unwrap_or_else (|error|{
        eprintln!("Error: {}", error);
        exit(1)
    });

    let double_numbers : Vec<Vec<i32>> =
        contents
        .trim()
        .split("\n")
        .map(|line|
    ...
    let mut first_numbers : Vec<_> = double_numbers.iter().map(|numbers| numbers.get(0).unwrap()).collect();
    let mut second_numbers : Vec<_> = double_numbers.iter().map(|numbers| numbers.get(1).unwrap()).collect();
    ...
    for first in first_numbers.iter() {
        let mut subtotal = 0;
        for second in second_numbers.iter() {
            if first == second {
                subtotal += 1;
            }
        }
